...
public class Predictor extends RegressionModelCalculator {
    ...
    @Override
    public void fitModels(Map<String, List<Tuple<Double, Double>>> processData) {

        processData.forEach((processName, data) -> {

            SimpleRegression regression = new SimpleRegression(true);

            for (Tuple<Double, Double> tuple : data) {

                regression.addData(tuple.getX(), tuple.getY());
            }
    ...
    // This implementation does not work with our use case. Now adapted in abstract class.

    // private static double[] calculateSimpleLinearRegression(double[] trainX, double[] trainY, double[] 6) {

    //     SimpleRegression regression = new SimpleRegression();
    //     double[] predicted = new double[trainY.length];


    //     for (int i = 0; i < trainX.length; i++) {
    //         regression.addData(trainX[i], trainY[i]);
    //     }
    ...
    //         predicted[i] = regression.predict(testX[i]);
    //     }
